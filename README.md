...
This is a separate repo from the course site, but **not a separate backend** — it shares one Firebase project (Authentication + Firestore) with `truck-talk-webapp`, which is what makes it "connected to the main platform": the same accounts, roles, and student progress data are visible from both.
...
- **`firestore.rules`** / **`firestore.indexes.json`** — the server-side access rules (the real security boundary, not the UI). Reference copies of the same rules used by `truck-talk-webapp`; deploy once to the shared Firebase project, from either repo.
...
1. Copy the same config object into [`shared/firebase-config.js`](shared/firebase-config.js) that you used in the course repo's copy.
...
- **Owner** — everything: approve/restrict anyone, assign teachers, edit any teacher's calendar. One hardcoded account (`abdujalilov7707@gmail.com`), enforced in `firestore.rules`.
